...
2. Edge layer on Raspberry Pi 4
   ...
   - Drives the optional RGB status LED from the current runtime state and
     immediate sensor/vision alerts.
...
The ultrasonic input also guards camera jumps. If a bbox center moves past
`safety.pixel_jump_threshold` while ultrasonic distance changes less than
`safety.ultrasonic_stable_delta_threshold_mm`, the edge runtime treats the bbox
as a likely false jump, shows the tracking-lost Red override, and does not pass
that bbox to the servo controller.

RGB status follows the state color by default. Alert colors override that state
color: infrared obstacle input shows Blue, and missing/rejected tracking shows
Red until a valid bbox is accepted or the operator starts a new command.
...
Run it with real PCA9685, distance sensors, and RGB status LED:
