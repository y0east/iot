...
from iot_servo_tracker.web.vision_validation_app import (
    ...
    build_validation_server_config,
)
...
class FullStackSimulationTests(unittest.TestCase):
    def test_web_command_reaches_edge_and_status_returns_to_web(self) -> None:
        ...
        self.assertEqual(events[0].vision_mode, "scripted")
    ...
    def test_production_mode_requires_real_webcam_frames(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "--production requires --webcam"):
            run_full_stack_simulation(
                AppConfig(),
                FullStackSimulationOptions(production=True, webcam=False, steps=1),
            )

    def test_web_validation_defaults_to_local_wedetect_adapter(self) -> None:
        server = build_validation_server_config(
            AppConfig().server,
            VisionValidationOptions(yolo_model="custom.pt", tracker="botsort.yaml"),
        )

        self.assertEqual(server.wedetect_ref_module, DEFAULT_WEDETECT_MODULE)
        self.assertEqual(server.yolo_model, "custom.pt")
        self.assertEqual(server.tracker, "botsort.yaml")
