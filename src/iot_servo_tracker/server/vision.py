...
class YoloePipeline:
    """Production inference pipeline using YOLOE/YOLO-World for open-vocabulary tracking."""

    def __init__(
        self,
        yolo_model: str = "yoloe26n.pt",
    ...
    def process_frame(self, ts_req: int, query: str, frame_bytes: bytes, frame_index: int = 0) -> TrackingResult:
        if query and query != self.active_query:
            try:
                ...
                print(f"Warning: Failed to set classes on YOLOE model: {e}")
            self.active_query = query
        ...
        best_bbox, best_conf, best_track_id = candidates[0]
    ...
    def redetect(self, frame_bytes: bytes, query: str, ts_req: int) -> TrackingResult:
        return self.process_frame(ts_req, query, frame_bytes)
